...
export class RouteRepository {
    ...
    async findRoutesBetweenNonTransactional(start: string, destination: string): Promise<void> {
        for (let i = 0; i < 100; i += 1) {
            await this.persistenceManager.query(
                new QuerySpecification<Route>()
                    .withStatement(this.routesBetween)
                    .bind([start, destination])
                    .transform(Route)
    ...
    @Transactional()
    async findRoutesBetweenTransactional(start: string, destination: string): Promise<void> {
        for (let i = 0; i < 3; i += 1) {
            await this.persistenceManager.query(
                new QuerySpecification<Route>()
                    .withStatement(this.routesBetween)
